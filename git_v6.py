...
class GitHubHelper:
    def __init__(self, root):
        ...
        # Добавляем переменную для отображения статуса подключения
    ...
    def setup_ui(self):
        ...
        connection_frame = ttk.Frame(info_frame)
        connection_frame.pack(fill="x", pady=(0, 5))
        ttk.Label(connection_frame, text="Статус подключения:", width=20).pack(side="left")
        self.connection_label = ttk.Label(connection_frame, textvariable=self.connection_status)
        ...
        self.file_tree = ttk.Treeview(tree_frame, columns=('status', 'description'), height=6)
