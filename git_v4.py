...
class GitHubHelper:
    def __init__(self, root):
        ...
        # Уменьшаем высоту окна для удобства
    ...
    def setup_ui(self):
        # Используем PanedWindow для разделения окна на две части
        main_pane = ttk.PanedWindow(self.root, orient=tk.HORIZONTAL)
        main_pane.pack(fill="both", expand=True, padx=10, pady=10)

        # Левая часть для основного контента
        left_frame = ttk.Frame(main_pane, padding="10")
        main_pane.add(left_frame, weight=3)  # Вес 3 для левой части

        # Правая часть для консоли и подсказок
        right_frame = ttk.Frame(main_pane, padding="10")
        main_pane.add(right_frame, weight=1)  # Вес 1 для правой части
        ...
        # ... (остальное содержимое system_frame остается без изменений) ...
        ...
        # Исправлено: добавлено wraplength для корректного отображения длинного URL
    ...
    # --- Методы класса (остаются без изменений, кроме check_remote_repository и push_to_github) ---
...
💡 СОВЕТ: Сохраняйте изменения часто и с понятными комментариями!
    ...
    # Измененный метод, чтобы он правильно парсил URL и показывал статус
    def check_remote_repository(self):
        ...
        if success and output.strip():
            ...
            if origins:
                ...
                self.remote_info.set(f"✅ {remote_url}")  # Отображаем полный URL
    ...
    # Измененный метод: добавлена проверка наличия удаленного репозитория
    ...
    def commit_all_changes(self):
        ...
        if add_success:
            ...
            if commit_success:
                ...
                self.log_message(f"❌ Ошибка при коммите:\n{commit_output}", 'error')
        else:
            self.log_message(f"❌ Ошибка при добавлении файлов:\n{add_output}", 'error')
    ...
    def commit_selected_file(self):
        ...
        if add_success:
            ...
            self.log_message(f"❌ Ошибка при добавлении файла:\n{add_output}", 'error')
